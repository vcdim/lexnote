...
\addbibresource{../gqbib/gq-biblatex.bib}
...
利用\textbf{重参数化技巧}，令 $\alpha_t = 1 - \beta_t$，
$\bar{\alpha}_t = \prod_{s=1}^{t} \alpha_s$，
我们可以\textit{直接从原始数据}$\mathbf{x}_0$采样任意时刻的$\mathbf{x}_t$，
而无需逐步迭代：
...
The \keyword{simplified loss} used in DDPM \cite{hoDenoisingDiffusionProbabilistic2020} reduces to a denoising objective:
...
\begin{rem}[Connection to Score Matching]
  ...
  \cite{songScoreBasedGenerativeModeling2020}.
